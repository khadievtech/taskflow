...
engine = create_async_engine(
    ...
    # echo сознательно НЕ используется: он навешивает собственный обработчик
    # логов поверх настроенного приложением, из-за чего каждый SQL-запрос
    # печатается дважды, причём вторая копия — обычным текстом в обход
    # JSON-форматтера. Уровень логирования SQL задаётся в app/core/logging.py
    # через стандартный logging, как и рекомендует документация SQLAlchemy.
    echo=False,
