...
- [~] Phase 6 — Auth
  - [x] 6a — собственный JWT: Argon2id, httpOnly cookie, защита эндпоинтов
  - [ ] 6b — Keycloak как централизованный провайдер
- [ ] Phase 7 — Kubernetes
...
## Аутентификация (Phase 6a)

Все эндпоинты задач требуют входа. Health-check намеренно остался публичным —
его опрашивают Docker и Kubernetes, у которых нет учётных данных.

**Первый запуск**: откройте приложение, нажмите «Нет аккаунта?
Зарегистрироваться», создайте учётную запись. После этого поставьте
`ALLOW_REGISTRATION=false` в `.env.prod` и пересоздайте backend — иначе любой,
кто дошёл до формы, сможет зарегистрироваться сам.

Технические решения и обоснования:

| Решение | Почему |
|---|---|
| **Argon2id** через `pwdlib` | `passlib` не поддерживается с 2020, использует удалённый в Python 3.13 модуль `crypt` и ломается на bcrypt 5.0 — при этом до сих пор фигурирует в документации FastAPI. Argon2id рекомендует OWASP; bcrypt молча обрезает пароль на 72 байтах |
| **PyJWT**, не `python-jose` | Развивается pyca (там же `cryptography`), у `python-jose` нет релизов |
| **httpOnly cookie**, не localStorage | Токен недоступен JavaScript, поэтому XSS его не украдёт. Взамен появляется риск CSRF, который закрывает `SameSite=Lax` |
| Алгоритм в `decode` передан списком явно | Иначе атакующий подставил бы `alg: none` или сменил алгоритм — классическая уязвимость реализаций JWT |
| Проверка пользователя в БД на каждом запросе | Подписанный токен подтверждает лишь факт выдачи. Без обращения к БД отключение учётной записи не действовало бы до истечения срока |
| Защита объявлена на уровне роутера | Новый эндпоинт нельзя случайно оставить открытым — secure by default |
| Отказ стартовать в production с ключом по умолчанию | Приложение с известным секретом лучше не запустить вообще, чем позволить подделывать токены |
...
**Важно**: `COOKIE_SECURE` пока `false`, потому что HTTPS нет. Флаг `Secure`
заставил бы браузер не отправлять cookie по HTTP, и вход не работал бы.
Обязательно включить после Phase 5b.
